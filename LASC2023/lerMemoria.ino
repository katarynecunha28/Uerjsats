...
void setup() {
  ...
  Wire.begin();  
  Serial.println();
  char i[] = {"ola pedro, tudo bem?"};
  char j[2];

  m.write(2, i);              //grava na memória a quantidade de bytes ocupada pelo tipo de dados passado
  
  int count = m.read(2, j);   //carrega da memória a quantidade de bytes 
